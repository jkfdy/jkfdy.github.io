---
title: Vibe coding AI编程经验汇总（持续更新中）
date: 2025-10-15 09:00:00 +0800
categories: [AI]
tags: [AI]
---

本文汇总了我日常关注的编程大神对于 Vibe coding（AI编程）的经验分享，你也可以把其他关于 Vibe coding 的实践分享给我，我会持续更新这个列表...

---

2026-01-14 更新：

决定后面还是以时间线（流水账）的方式更新这个阅读列表，并加入一点我的“旁白”，所以这篇文章可能更适合倒着读。

斯坦福大学课程：[CS146S：现代软件开发人员](https://themodernsoftware.dev/)，掌握现代开发工具，理解人工智能辅助编程，学习自动化测试与部署，探索新兴软件趋势。

2026 年之前的列表归档：

* [MCP 是什么，现状和未来](https://onevcat.com/2025/02/mcp/)
* [一个半月高强度 Claude Code 使用后感受](https://onevcat.com/2025/08/claude-code/)
* [Magpie 和 「AI 贼船」- 再谈 vibe coding，当代码变得廉价时...](https://onevcat.com/2025/10/magpie-and-ai-ship/)
* [上了AI的贼船](https://ai.onev.cat)
* [AI 辅助编程的崛起意味着什么？](https://limboy.me/posts/ai-programming)
* [关于 AI 辅助编程的一些实践和思考](https://limboy.me/posts/ai-assisted-coding)
* [Vibe Coding 一段时间后的感受](https://limboy.me/posts/vibe-coding)

2025-12-21: [上了AI的贼船](https://ai.onev.cat)更新了《告别 MCP，回归 CLI》。同时 onvcat 还翻译并发布了中文版[命令行界面设计指南](https://clig.onev.dev)

2025-12-31: Simon Willison（Django框架的共同创造者、Lanyrd联合创始人）更新了[the year in llms](https://simonwillison.net/2025/Dec/31/the-year-in-llms/)，池建强发布了[翻译版本](https://note.mowen.cn/detail/lypEigmjeAU1fykX5n5Fq)，此文中也提到了 MCP 可能会被 CLI 取代的观点。

2026-1-11: limboy 更新了[「共」型人才：AI 时代的个体进化论](https://limboy.me/posts/gong-type-of-talent)，limboy 写的文章总是那么有深度，真不知道是怎么做到既有深度，更新频率又高的。
